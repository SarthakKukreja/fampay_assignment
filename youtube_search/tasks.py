...
# Keep calling YouTube API and updating database continuously in background periodicly and asynchronously as celery task.
@shared_task
def updateDatabase():
    publishedAfter = None
    ...
    if( VariableStorage.objects.filter(key='publishedAfter').exists()):
        ...
        # Update 'last updated' time variable so that future calls need not fetch
        # already existing data.
        publishedAfter.value = convert_time()
        publishedAfter.save()

    # DB is unpopulated so fetch all data from api. Save 
    else:
